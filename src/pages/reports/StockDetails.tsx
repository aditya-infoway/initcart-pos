...
/**
 * ✅ Safe per-unit price calculation
 * Handles: null/undefined billAmount, qty=0, division by zero
 */
const perUnitPrice = (qty: number, billAmount: number): number | null => {
...
const TYPE_COLOURS: Record<string, string> = {
  ...
  "Stock Transfer (Received)": "bg-teal-100 text-teal-800",
  "Stock Transfer (Sent)": "bg-cyan-100 text-cyan-800",
  "Stock Return (Sent)": "bg-purple-100 text-purple-800",
  "Stock Return (Received)": "bg-indigo-100 text-indigo-800",
  "B2B Transfer (Received)": "bg-emerald-100 text-emerald-800",
  "B2B Transfer (Sent)": "bg-lime-100 text-lime-800",
  "B2B Return (Sent)": "bg-fuchsia-100 text-fuchsia-800",
  "B2B Return (Received)": "bg-violet-100 text-violet-800",
  "B2B Sale (Sent)": "bg-orange-100 text-orange-800",
};
...
const typeBadge = (t: string) => {
  ...
  return (
    <span
      className={`text-xs font-semibold px-2 py-0.5 rounded-full whitespace-nowrap ${cls}`}
    >
      {t}
    </span>
  );
...
const StockDetail: React.FC = () => {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get(
        `stock-history/${variantId}/?${params.toString()}`,
        { headers: authHeader() }
      );

      const data: StockHistoryEntry[] = Array.isArray(res.data)
        ? res.data
        : [];

      // ✅ Sanitize every row so no NaN / undefined reaches UI
      ...
      const nameRow = sanitized.find((r) => r.type !== "Opening");
  ...
  const exportToExcel = () => {
    ...
    const openingStockVal = openingRow ? openingRow.qty : 0;
    const currentStockVal = lastRow ? lastRow.currentStock : 0;

    const totalPurchasedVal = history
      .filter((r) => r.type === "Purchase")
      .reduce((s, r) => s + r.qty, 0);

    const totalPurchaseReturnsVal = history
      .filter((r) => r.type === "Purchase Return")
      .reduce((s, r) => s + Math.abs(r.qty), 0);

    const totalSoldVal = history
      .filter((r) => r.type === "Sale (POS)" || r.type === "Sale (Website)")
      .reduce((s, r) => s + Math.abs(r.qty), 0);

    const totalSalesReturnsVal = history
      .filter((r) => r.type === "Sales Return")
      .reduce((s, r) => s + r.qty, 0);

    const totalInward = totalPurchasedVal + totalSalesReturnsVal;
    const totalOutward = totalPurchaseReturnsVal + totalSoldVal;
    const netMovement = totalInward - totalOutward;

    const exportData = history.map((row, index) => {
      const unitPrice = perUnitPrice(row.qty, row.billAmount);
      return {
        "SR No": index + 1,
        Date: row.date
        ...
        "Purchase Price (₹)": unitPrice !== null ? unitPrice.toFixed(2) : "0.00",
        "Bill Amount (₹)": (Number(row.billAmount) || 0).toFixed(2),
    ...
    const summaryData = [
      ...
      ["----------------------------------------"],
      ["Opening Stock:", openingStockVal],
      [],
      [" INWARD MOVEMENT:"],
      ["  - Total Purchased:", totalPurchasedVal],
      ["  - Total Sales Returns:", totalSalesReturnsVal],
      ["  - Total Inward:", totalInward],
      [],
      [" OUTWARD MOVEMENT:"],
      ["  - Total Purchase Returns:", totalPurchaseReturnsVal],
      ["  - Total Sold:", totalSoldVal],
      ...
      [],
      [" CLOSING STOCK:"],
      ["  - Current Stock:", currentStockVal],
      [
        "  - Formula: Opening + Purchases - Purchase Returns - Sold + Sales Returns",
      ],
      [],
      ["========================================"],
    ...
    XLSX.utils.book_append_sheet(
      wb,
      ws,
      `Stock_History_${(itemName || variantId || "Item").slice(0, 25)}`
    );
  ...
  const handlePrint = () => {
    ...
    const rows = history
      .map((r, i) => {
        const unitPrice = perUnitPrice(r.qty, r.billAmount);
    ...
    w.document.write(`<html><head><title>Stock History - ${itemName || variantId}</title>
      ...
      <table><thead><tr>
        ...
        <th>Qty</th><th>Bill Amount</th><th>Purchase Price</th><th>Current Stock</th>
    ...
    <div className="bg-gray-50 min-h-screen p-4 font-sans">
      ...
      {!loading && history.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
          ...
          <div className="bg-orange-50 rounded-xl p-3 text-center shadow">
            ...
            <p className="font-bold text-orange-700 mt-1 text-lg">
              {fmt(totalSold)}
            </p>
          </div>
          <div
            className={`rounded-xl p-3 text-center shadow ${
              currentStock < 10 ? "bg-red-50" : "bg-purple-50"
            }`}
          >
            <p
              className={`text-xs font-medium ${
                currentStock < 10 ? "text-red-500" : "text-purple-500"
              }`}
            >
              Current Stock
            </p>
            <p
              className={`font-bold mt-1 text-lg ${
                currentStock < 10 ? "text-red-700" : "text-purple-700"
              }`}
            >
              {fmt(currentStock)}
              {currentStock < 10 && (
                <span className="ml-1 text-xs font-normal bg-red-200 text-red-700 px-1.5 py-0.5 rounded-full">
                  Low
                </span>
              )}
            </p>
      ...
      {history.length > 0 && (
        <div className="bg-white rounded-xl shadow p-3 mb-4 flex justify-between items-center flex-wrap gap-3">
          <span className="text-sm text-gray-600">{totalRows} total entries</span>
          <div className="flex items-center gap-2">
      ...
      {/* ── Error ────────────────────────────────────────────────────────── */}
      {error && (
        <div className="bg-red-50 border border-red-200 text-red-700 rounded-xl p-4 mb-4 text-sm flex items-center justify-between">
          <span>⚠️ {error}</span>
          <button
            ...
            <tbody>
              {pagedRows.length === 0 ? (
                <tr>
                  ...
                  >
                    {error ? "Could not load data" : "No stock history found"}
                  </td>
                </tr>
              ) : (
              ...
              {(page === totalPages || totalPages <= 1) &&
                history.length > 0 && (
                  <tr className="bg-indigo-50 font-bold border-t-2 border-indigo-300">
                    ...
                    <td
                      colSpan={2}
                      className="p-3 border text-right text-gray-600 text-sm"
                    >
                      {totalRows} entries
                    </td>
                    <td className="p-3 border text-right">
                      <span
                        className={
                          currentStock < 10 ? "text-red-700" : "text-blue-700"
                        }
                      >
                        {fmt(currentStock)}
                      </span>
