...
interface CashReceiptFormValues {
...
const validationSchema = Yup.object({
  cashAccount: Yup.number().required("Cash Account is required"),
...
const CashReceipt: React.FC = () => {
  ...
  // ✅ PERMISSIONS
  const { canAdd } = usePermission("/Cash-receipt");
...
useEffect(() => {
  ...
  const exportToExcel = () => {
    ...
    const exportData: any[] = filteredRows.map((row, index) => ({
      ...
      "Type": row.type || "CR",
      "Cash Account": row.cash_account_name || row.cash_account || "-",
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Cash Receipt Register");
    const fileName = `Cash_Receipt_Register_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  const fetchReceipts = async (page = 1) => {
    ...
    try {
      ...
      const res = await api.get<PaginatedResponse>(`cash-receipts/?page=${page}&page_size=${pageSize}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleCashReceiptSubmit = async (values: any, { resetForm, setSubmitting }: any) => {
    try {
      if (values.selectedBill && values.receiptType === 'salesEntry') {
        ...
        const res = await api.post("/receive-sales-credit-bill-cash/", salesPayload);
      ...
      if (values.selectedBill && values.receiptType === 'purchaseReturn') {
        ...
        const res = await api.post("/receive-purchase-return-credit-bill-cash/", purchaseReturnPayload);
      ...
      if (values.selectedBill && values.receiptType === 'stockTransfer') {
        ...
        const res = await api.post("/receive-stock-transfer-bill-cash/", stockTransferPayload);
      ...
      if (values.selectedBill && values.receiptType === 'b2bSale') {
        ...
        const res = await api.post("/receive-b2b-sale-bill-cash/", b2bSalePayload);
      ...
      if (values.selectedBill && values.receiptType === 'stockReturn') {
        ...
        const res = await api.post("/receive-stock-return-bill-cash/", stockReturnPayload);
      ...
      const res = await api.post("cash-receipts/", payload);
      fetchReceipts(currentPage);
      resetForm();
      setOpen(false);
      toast.success("Cash receipt saved successfully");
    } catch (err: any) {
      const errorResponse = err.response?.data as ApiErrorResponse;
      let message = "Failed to save cash receipt ❌";
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold">Cash Receipt Register</h1>
      ...
      <div className="mb-4 flex flex-wrap gap-3 items-center">
        ...
        >
          ...
          <option value="PRCR">PRCR</option>
          {isSuperAdmin && <option value="STCR">STCR</option>}
          {isSuperAdmin && <option value="B2BCR">B2BSCR</option>}
          {isBranch && <option value="STRCR">STRCR</option>}
        </select>
        ...
        <table className="w-full text-sm">
          <thead className="bg-blue-600 text-white text-center">
            <tr>
              ...
              <th className="p-3 border border-gray-200 whitespace-nowrap">Cash Account</th>
            ...
            {currentItems.length > 0 ? (
              currentItems.map((r, i) => (
                <tr key={r.id || i} className="border-t hover:bg-gray-50">
                  ...
                  <td className="p-3 border border-gray-200 whitespace-nowrap">{r.cash_account_name || r.cash_account || "-"}</td>
                  ...
                  <td className="p-3 border border-gray-200 whitespace-nowrap">{r.created_by_name || "-"}</td>
                  <td className="p-3 border border-gray-200 whitespace-nowrap">
                    <button
                      type="button"
                      onClick={() => printReceipt(r, "cash")}
                ...
                useEffect(() => {
                  if (open) {
                    const fetchVoucher = async () => {
                      try {
                        const res = await api.get(`voucher/generate/?type=CR`);
      ...
      <label className="flex items-center gap-2">
        ...
        <span>Stock Return</span>
                        ...
                        <div className="bg-gray-50 p-4 rounded-lg border">
                          ...
                          {values.selectedBill && (
                            <div className="mt-3 p-2 bg-green-50 rounded text-sm">
                              <p><strong>Bill No:</strong> {values.selectedBill.billNo}</p>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <AccountSelect label="Cash Account" name="cashAccount" />
