...
const CashPayment: React.FC = () => {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Cash Payment Register");
    
    // Generate filename with current date
    const fileName = `Cash_Payment_Register_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  const fetchPayments = async (page = 1) => {
    ...
    try {
      ...
      const res = await api.get<PaginatedResponse>(`cash-payments/?page=${page}&page_size=${pageSize}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleCashPaymentSubmit = async (values: any, { resetForm, setSubmitting }: any) => {
    try {
      ...
      if (values.selectedBill && values.paymentType === 'purchaseEntry') {
        ...
        const res = await api.post("/pay-purchase-credit-bill-cash/", purchasePayload);
      ...
      if (values.selectedBill && values.paymentType === 'salesReturn') {
        ...
        const res = await api.post("/settle-credit-bill/", settlePayload);
      ...
      if (values.selectedBill && values.paymentType === 'stockReceived') {
        ...
        const res = await api.post("/pay-stock-received-bill-cash/", stockPayload);
      ...
      if (values.selectedBill && values.paymentType === 'stockReturn') {
        ...
        const res = await api.post("/pay-stock-return-bill-cash/", stockReturnPayload);
      ...
      if (values.selectedBill && values.paymentType !== 'manual') {
        payload.narration = `${payload.narration} - Payment against ${values.paymentType === 'purchaseEntry' ? 'Purchase Entry Credit Bill' : 'Bill'}: ${values.selectedBill.billNo}`;
      }

      const res = await api.post("cash-payments/", payload);
      fetchPayments(currentPage);
      resetForm();
      setOpen(false);
      ...
      let message = "Failed to save cash payment ❌";
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold">Cash Payment Register</h1>
      ...
      <div className="mb-4 flex flex-wrap gap-3 items-center">
        ...
        >
          ...
          {!isSuperAdmin && <option value="STCP">STCP</option>}
          {isSuperAdmin && <option value="STRCP">STRCP</option>}
        </select>
              ...
              currentItems.map((r, i) => (
                <tr key={r.id || i} className="border-t hover:bg-gray-50">
                  ...
                  <td className="p-3 border border-gray-200 whitespace-nowrap">
                    <span className={`px-2 py-1 rounded text-xs font-semibold ${r.type === "CP"
                ...
                useEffect(() => {
                  if (open) {
                    const fetchVoucher = async () => {
                      try {
                        const res = await api.get(`voucher/generate/?type=CP`);
