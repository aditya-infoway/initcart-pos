...
import {
  ...
  FaCalendarAlt, FaFileInvoice, FaEdit, FaFilter,
} from "react-icons/fa";
      ...
      <div className="flex gap-2 flex-wrap justify-center">
        <button
          ...
          className={`px-3 py-1 rounded transition flex items-center gap-1 ${
            !previous ? "bg-gray-300 cursor-not-allowed" : "bg-blue-600 text-white hover:bg-blue-700"
          }`}
        >
        ...
        <button
          ...
          disabled={!next}
          className={`px-3 py-1 rounded transition flex items-center gap-1 ${
            !next ? "bg-gray-300 cursor-not-allowed" : "bg-blue-600 text-white hover:bg-blue-700"
          }`}
        >
...
export default function StockReturn() {
  ...
  const [toBranchName, setToBranchName] = useState("Company");
  ...
  const [current, setCurrent] = useState<{
    ...
    verifiedItemId: null, item_name: "", variant_info: "", barcode: "",
  ...
  const loadReturns = useCallback(async (page = 1) => {
    ...
    try {
      ...
      let data: ReturnListItem[] = results.data || [];
...
const fetchReturnNo = async () => {
  ...
  const fetchToBranch = async () => {
    try {
      ...
      if (res.data?.company_branch_name) setToBranchName(res.data.company_branch_name);
      else if (res.data?.to_branch_name) setToBranchName(res.data.to_branch_name);
    } catch {
      // keep default "Company"
    }
  };
...
const loadModalItems = useCallback(async (page = 1) => {
  ...
  try {
    const params = new URLSearchParams({ page: String(page) });
    if (debouncedSearch) params.append("search", debouncedSearch); // Use debouncedSearch instead of modalSearch
    const res = await api.get(`stock-returns/verified-items/?${params}`);
    ...
    toast.error("Could not load verified items");
  }
...
const openSelectModal = () => {
  ...
  // ── Pick an item from modal → fills the entry row, closes modal ──
  const pickItem = (item: VerifiedItem) => {
    // Already added items reduce available qty for that same verified item
    const alreadyAdded = addedItems.find((a) => a.verifiedItemId === item.id);
  ...
  // ── Add current row into the added-items list (below) ──
  const handleAddItem = () => {
    if (!current.verifiedItemId) {
    ...
    setAddedItems((prev) => {
      const existingIdx = prev.findIndex((a) => a.verifiedItemId === current.verifiedItemId);
      if (existingIdx >= 0) {
        // merge into existing row for same verified item
        const next = [...prev];
        next[existingIdx] = {
          ...next[existingIdx],
      ...
      return [
        ...
        {
          uid: uidCounter,
          verifiedItemId: current.verifiedItemId!,
  ...
  const createReturn = async () => {
    ...
    setCreating(true);
    try {
      const res = await api.post("stock-returns/create-from-items/", {
  ...
  const updatePackaging = async (returnId: number, itemIds: number[], isReady: boolean) => {
    if (isReady) {
      const confirmed = await showConfirm(
        `Are you sure you want to mark ${itemIds.length} item(s) as packaged?`
      );
      if (!confirmed) return;
    }
    setProcessingAction(true);
    try {
      const res = await api.post(`stock-returns/${returnId}/packaging/`, {
        item_ids: itemIds,
        is_packaging_ready: isReady,
      });
      if (res.data.success) {
  ...
  const cancelReturn = async (id: number) => {
    const confirmed = await showConfirm("Cancel this return request?");
    if (!confirmed) return;
    try {
      const res = await api.post(`stock-returns/${id}/cancel/`);
      if (res.data.success) {
        toast.success(res.data.message);
        setTab("list");
        loadReturns(1);
  ...
  // ── Render: List view (Purchase-Register style) ──
  if (tab === "list") {
    return (
      <div className="p-6 bg-white min-h-screen">
        ...
        <div className="flex justify-between items-center mb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-800">Stock Return Register</h1>
              <p className="text-xs text-gray-400">Return verified items to company branch</p>
            </div>
          </div>
          <button
          ...
          <div className="flex flex-wrap gap-3 items-center justify-between">
            <div className="flex gap-2 flex-1">
              <div className="relative flex-1 max-w-md">
                ...
                {listSearch && (
                  <button
                    onClick={() => { setListSearch(""); loadReturns(1); }}
        ...
        <div className="mb-4 text-sm text-gray-600">
          Showing {returns.length} of {returnsPagination.count} return records
        ...
        <div className="overflow-x-auto bg-white rounded-lg shadow">
          <table className="w-full text-sm">
            <thead className="bg-gradient-to-r from-blue-600 to-blue-500 text-white">
              <tr>
                {["SR", "Return No", "To Branch", "Date", "Items", "Total Qty", "Note", "Status", "Action"].map((h) => (
              ...
              {loading ? (
                ...
                returns.map((r, idx) => (
                  <tr key={r.id} className="border-b hover:bg-blue-50 transition">
                    ...
                    <td className="p-3 border border-gray-200 whitespace-nowrap max-w-xs truncate">{r.note || "-"}</td>
                    <td className="p-3 border border-gray-200 whitespace-nowrap">
  ...
  // ── Render: Create view (Purchase-Entry style) ──
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-6 pb-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <button
            onClick={() => { setTab("list"); setAddedItems([]); }}
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-9 gap-3 items-end">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  disabled={!current.verifiedItemId}
                  onChange={(e) => {
              ...
              <DisplayField
                label="Amount ₹"
                value={
                  current.verifiedItemId
                    ? (Number(current.quantity || 0) * current.rate).toFixed(2)
                    : "-"
                }
              />
            ...
            {current.verifiedItemId && (
              <div className="mt-3 text-xs text-gray-400">
                From transfer <b className="text-gray-600">{current.transfer_no}</b> · Branch:{" "}
                <b className="text-gray-600">{current.from_branch_name}</b>
              </div>
            )}
          </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white shadow-lg border-t p-3 flex gap-3 justify-center z-10">
          <button
            type="button"
            onClick={() => { setAddedItems([]); setUidCounter(1); }}
          ...
          <button
            ...
            disabled={creating || addedItems.length === 0}
              ...
              >
                <div className="flex justify-between items-center px-6 py-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white flex-shrink-0">
                  <h3 className="text-xl font-semibold flex items-center gap-2">
                    <FaBox /> Select Verified Item to Return
                  </h3>
                    ...
                    <table className="w-full text-sm min-w-[900px]">
                      ...
                      <tbody>
                        {modalLoading ? (
                          ...
                          modalItems.map((row) => {
                            const alreadyAdded = addedItems.find((a) => a.verifiedItemId === row.id);
                            const remaining = row.quantity - (alreadyAdded ? alreadyAdded.quantity : 0);
                            const disabled = remaining <= 0;
                            return (
                              <tr key={row.id} className={`border-b hover:bg-gray-50 transition ${disabled ? "opacity-40" : ""}`}>
                                <td className="px-3 py-2 text-center">
                                  <button
                                    type="button"
                                    disabled={disabled}
                                ...
                                <td className="px-3 py-2">{row.transfer_no}</td>
                                <td className="px-3 py-2">{row.from_branch_name}</td>
...
function ReturnDetailView({
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      {/* Packaging Actions - Only visible when status is 'approved' */}
      {canUpdatePackaging && (
        <div className="px-6 py-3 border-b bg-blue-50/30 flex flex-wrap items-center gap-3 justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-lg ${
              allPackaged ? "bg-emerald-100 text-emerald-700" : "bg-blue-100 text-blue-700"
            }`}>
          ...
          <div className="flex gap-2">
            ...
            {packagingItems.size > 0 && (
              <button
                onClick={() => handlePackagingUpdate(true)}
                disabled={processing}
      ...
      {/* Status Message - If not approved, show message */}
                ...
                <motion.tr key={item.id}
                  ...
                  {canUpdatePackaging && !item.is_returned_to_company && (
                    <td className="px-3 py-3 text-center">
                      <button
                        onClick={() => togglePackagingItem(item.id)}
                        className={`w-7 h-7 rounded-lg flex items-center justify-center transition-colors ${
                          isSelected ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-400 hover:bg-gray-300"
                        }`}
                      >
