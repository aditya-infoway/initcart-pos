...
const PurchaseEntryReport: React.FC = () => {
  ...
  const fetchItems = async (branchId?: string) => {
    ...
    try {
      ...
      const mapped = itemsArray.map((item: any, index: number) => ({
        uid: index, id: item.id, billNo: item.billNo, date: item.date,
        due_date: item.dueDate, grand_total: item.grand_total,
        total_basic: item.total_basic, total_tax: item.total_tax,
        terms: item.terms, narration: item.narration,
        party_name_name: item.party_name_name, purchasebill_no: item.purchasebill_no,
  ...
  useEffect(() => {
    ...
    if (searchTerm.trim()) {
      ...
      f = f.filter(i => i.billNo?.toLowerCase().includes(t) || i.party_name_name?.toLowerCase().includes(t) || i.purchasebill_no?.toLowerCase().includes(t));
    }
  ...
  const exportToExcel = () => {
    const rows = filteredItems.map((item, idx) => ({
      ...
      "Purchase Bill No": item.purchasebill_no || "-", "Due Date": item.due_date || "-",
    ...
    rows.push({ "SR No": "" as any, "Date":"","Terms":"","Party Name":"TOTAL","Bill No":"",
      "Purchase Bill No":"","Due Date":"","Narration":"",
    ...
    ws["!cols"] = [{wch:6},{wch:12},{wch:10},{wch:28},{wch:18},{wch:20},{wch:12},{wch:32},{wch:16},{wch:14},{wch:12},{wch:16}];
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Purchase Report");
    XLSX.writeFile(wb, `Purchase_Entry_Report_${new Date().toISOString().slice(0,10)}.xlsx`);
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-screen-2xl mx-auto px-6 py-5">
        ...
        <div className="no-print bg-white rounded-xl border border-gray-200 shadow-sm p-4 mb-5">
          <div className="flex flex-wrap gap-3 items-center">
            <div className="relative flex-1 min-w-[220px] max-w-lg">
              ...
              <input type="text" placeholder="Search by Bill No, Party, Purchase Bill No…" value={searchTerm}
        ...
        <div className="hidden print:block mb-6 text-center border-b-2 border-gray-800 pb-4">
          <h1 className="text-2xl font-bold">PURCHASE ENTRY REPORT</h1>
                ...
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="bg-slate-700 text-white text-xs uppercase tracking-wide">
                      {["SR","Date","Terms","Party Name","Bill No","Purchase Bill No","Due Date","Narration","Total Basic","Total Tax","F+O+R","Grand Total","Items"].map(h => (
                  ...
                  <tbody>
                    {paginatedItems.length === 0 ? (
                      <tr><td colSpan={13} className="text-center py-20 text-gray-400 text-sm">
                        {searchTerm || hasActiveFilters() ? "No records match your filters." : "No purchase records found."}
                      </td></tr>
                    ) : (
                      paginatedItems.map((item, index) => (
                        <tr key={item.id} className={`transition hover:bg-blue-50 ${index % 2 === 0 ? "bg-white" : "bg-slate-50/40"}`}>
                          ...
                          <td className="px-3 py-2.5 whitespace-nowrap text-gray-600">{item.purchasebill_no||"-"}</td>
                  ...
                  <tfoot>
                    <tr className="bg-slate-50 border-t-2 border-slate-300">
                      <td colSpan={8} className="px-3 py-3 text-sm font-bold text-gray-700">TOTAL — {filteredItems.length} records</td>
            ...
            <div className="overflow-x-auto max-h-[55vh]">
              <table className="w-full text-sm">
                <thead className="bg-gray-100 sticky top-0">
                  <tr>
                    {["SR","Item Name","HSN","Qty","Alt Qty","Price","Per","Disc%","Basic Amt","Disc Amt","Tax Amt","Net Value"].map(h=>(
                  ...
                  {variants.length === 0 ? (
                    ...
                    variants.map((v:any, i:number) => (
                      <tr key={i} className={`hover:bg-blue-50 transition ${i%2===0?"bg-white":"bg-gray-50/40"}`}>
                        ...
                        <td className="px-3 py-2 font-medium text-gray-800 whitespace-nowrap">{v.itemName_name??"-"}</td>
                        <td className="px-3 py-2 text-gray-500">{v.hsnCode??"-"}</td>
                        <td className="px-3 py-2 text-right">{v.quantity??"-"}</td>
                        <td className="px-3 py-2 text-right">{v.altQuantity??"-"}</td>
                        <td className="px-3 py-2 text-right">₹{v.price??"-"}</td>
                        <td className="px-3 py-2">{v.per??"-"}</td>
                        <td className="px-3 py-2 text-right">{v.discountPercent??"-"}</td>
                        <td className="px-3 py-2 text-right">₹{v.basicAmount??"-"}</td>
                        <td className="px-3 py-2 text-right">₹{v.discountAmount??"-"}</td>
                        <td className="px-3 py-2 text-right">₹{v.taxAmount??"-"}</td>
                        <td className="px-3 py-2 text-right font-semibold text-gray-900">₹{v.netValue??"-"}</td>
                      </tr>
                    ))
                  )}
                </tbody>
...
export default PurchaseEntryReport;