...
interface SalesReturn {
  id: number; return_no: string; date: string; customer_name: string;
...
const SalesReturnReport: React.FC = () => {
  ...
  const fetchReturns = async (branchId?: string) => {
    setLoading(true);
    try {
      let url = `sales-return-list/?page=1&page_size=1000`;
      if (branchId) {
        url += `&branch_id=${branchId}`;
      }
      
      console.log("🔄 Fetching sales returns with URL:", url);
      ...
      console.log(`✅ Loaded ${all.length} sales returns for branch:`, branchId || 'default');
  ...
  useEffect(() => {
    ...
    if (searchTerm.trim()) {
      ...
      f = f.filter(i => i.return_no?.toLowerCase().includes(t) || i.customer_name?.toLowerCase().includes(t) || i.reason_for_return?.toLowerCase().includes(t));
    }
    if (filters.return_type) f = f.filter(i => i.return_type?.toLowerCase() === filters.return_type.toLowerCase());
    if (filters.party) f = f.filter(i => i.customer_name?.toLowerCase().includes(filters.party.toLowerCase()));
  ...
  const exportToExcel = () => {
    const rows = filteredReturns.map((item, idx) => ({
      ...
      "Party Name": item.customer_name||"-", "Reason": item.reason_for_return||"-",
      ...
      <div className="no-print sticky top-0 z-30 bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-screen-2xl mx-auto px-6 py-3 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-800 leading-tight">Sales Return Report</h1>
                  ...
                  <tbody>
                    {paginatedReturns.length === 0 ? (
                      <tr><td colSpan={10} className="text-center py-20 text-gray-400 text-sm">
                        {searchTerm||hasActiveFilters()?"No records match your filters.":"No sales return records found."}
                      </td></tr>
                    ) : (
                      paginatedReturns.map((item, index) => (
                        <tr key={item.id} className={`transition hover:bg-blue-50 ${index%2===0?"bg-white":"bg-slate-50/40"}`}>
                          ...
                          <td className="px-3 py-2.5 whitespace-nowrap font-medium text-gray-800">{item.customer_name||"-"}</td>
          ...
          <div className="bg-white w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden">
            <div className="flex justify-between items-center px-6 py-4 bg-slate-700 text-white">
              <div>
                ...
                <p className="text-xs text-slate-300 mt-0.5">{selectedReturn.customer_name} | {selectedReturn.date}</p>
              </div>
